 ...
 * @version 3.0.0 (Muestreo Inteligente y Corrección de Zona Horaria Definitiva)
 * @since 1.0.0
 * @copyright SENA 2025
 */
...
export class DashboardService {
	 ...
	 * @description Obtiene datos históricos aplicando un muestreo inteligente y manejo de zona horaria.
	 * @param {DashboardFiltersDto} filters - Filtros de la solicitud, incluyendo rango de fechas y tanque.
	 * @param {User} user - El usuario que realiza la solicitud.
	 * @returns {Promise<Record<SensorType, { time: string; value: number }[]>>} - Datos históricos formateados.
	 */
	async getHistoricalData(filters: DashboardFiltersDto, user: User) {
		...
		const fromDate = new Date(`${startDate}T00:00:00.000-05:00`);
		const toDate = new Date(`${endDate}T23:59:59.999-05:00`);
		...
		for (const type of sensorTypesToFetch) {
			...
			if (rawData.length > 0) {
				if (rawData.length <= maxChartPoints) {
					...
					if (
						sampled[sampled.length - 1].id !== rawData[rawData.length - 1].id
					) {
						sampled.push(rawData[rawData.length - 1]);
					}
					formattedData[type] = sampled.map((d) => ({
						time: d.timestamp.toISOString(),
						value: d.value,
					}));
				}
