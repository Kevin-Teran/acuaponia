...
// --- FUNCIÓN AUXILIAR PARA GENERAR TOKENS (VERSIÓN MÁS EXPLÍCITA) ---
const generateTokens = (user: { id: string; email: string; role: string; }) => {
  ...
  // --- CORRECCIÓN FINAL: Se usan valores fijos para aislar el problema de tipos ---
  // Esto elimina cualquier ambigüedad relacionada con process.env.
...
// POST /api/auth/login
router.post('/login', authValidation.login, asyncHandler(async (req: Request, res: Response) => {
...
router.post('/refresh', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (typeof jwtRefreshSecret !== 'string') {
    logger.error('JWT_REFRESH_SECRET no está definida en el entorno.');
    throw new CustomError('Error de configuración del servidor.', 500);
  }
...
router.get('/me', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (typeof jwtSecret !== 'string') {
    logger.error('JWT_SECRET no está definida en el entorno.');
    throw new CustomError('Error de configuración del servidor.', 500);
  }
