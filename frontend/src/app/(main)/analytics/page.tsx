...
const AnalyticsPage = () => {
  ...
  // 1. Se llama a tu hook existente. Se extrae `users` (según tu definición).
  const { tanks, users, loading: isInfraLoading, fetchDataForUser } = useInfrastructure(isAdmin);
  ...
  // Efecto A: Se establece el `selectedUserId` inicial.
  useEffect(() => {
    if (currentUser) {
      setSelectedUserId(isAdmin ? null : currentUser.id);
    }
  }, [currentUser, isAdmin]);

  // Efecto B: Se cargan los tanques. ESTA ES LA CORRECCIÓN CLAVE.
  // Solo se llama a `fetchDataForUser` si `selectedUserId` tiene un valor.
  // Esto previene la llamada con `null` que causaba el warning.
  useEffect(() => {
    if (selectedUserId) {
  ...
  // Efecto C: Reacciona a la carga de tanques.
  useEffect(() => {
  ...
  // Efecto D: Carga los datos de analíticas al final.
  useEffect(() => {
    if (selectedUserId && filters.tankId) {
      const fullFilters = { ...filters, userId: selectedUserId };
      fetchData(fullFilters);
    }
  }, [selectedUserId, filters, fetchData]);
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-4 md:p-6 bg-slate-50 dark:bg-slate-900 min-h-full">
      <aside className="lg:w-1/4 xl:w-1/5 lg:sticky lg:top-6 h-fit">
        <AnalyticsControlPanel
          ...
          tanksList={tanks || []}
          usersList={users || []} // Se pasa `users` que viene de tu hook.
      ...
      <main className="flex-1 space-y-6">
        { !isInfraLoading && filters.tankId ? (
          <>
            {/* ... el resto del JSX para mostrar las gráficas ... */}
            <section>
              <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 mb-4">
                Análisis Principal: <span className="font-bold text-green-600 dark:text-green-400">{filters.sensorType}</span>
              </h2>
          ...
          <div className="flex flex-col items-center justify-center h-full bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8 text-center">
            <BrainCircuit className="w-20 h-20 text-green-400 mb-6 animate-pulse" />
            <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Listo para Analizar</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm">
              {isInfraLoading ? 'Cargando datos...' : (isAdmin && !selectedUserId ? 'Por favor, selecciona un usuario para empezar.' : 'Selecciona un tanque para visualizar los datos.')}
            </p>
          </div>
        )}
      </main>
