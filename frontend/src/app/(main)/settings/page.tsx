 ...
 * La lógica está unificada en este archivo para simplicidad.
 */
 'use client';
 ...
 const NotificationSettingsTab = () => {
     ...
     const SettingRow = ({ title, desc, children }: any) => (<div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800/50"><div><h3 className="font-medium">{title}</h3><p className="text-sm text-gray-600 dark:text-gray-400">{desc}</p></div><div>{children}</div></div>);
   ...
   return (
     <div className="space-y-6 animate-in fade-in duration-500">
       ...
       <div className="flex border-b border-gray-200 dark:border-gray-700">
         {tabs.map(tab => (
           <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={clsx('flex items-center gap-2 px-4 py-3 font-medium text-sm transition-colors', activeTab === tab.id ? 'border-b-2 border-green-500 text-green-600' : 'border-b-2 border-transparent text-gray-500 hover:text-gray-800')}>
