 ...
 * @description Versión final y robusta para la gestión de Tanques y Sensores.
...
const getStatusChip = (status: Tank['status'] | Sensor['status']) => {
    const styles: Omit<Record<TankStatus | Sensor['status'], string>, 'ERROR'> = {
        ACTIVE: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
        MAINTENANCE: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300',
        INACTIVE: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
    };
    const text: Omit<Record<TankStatus | Sensor['status'], string>, 'ERROR'> = { 
        ACTIVE: 'Activo', 
...
export default function DevicesPage() {
    ...
    useEffect(() => {
        ...
        if (selectedUserId && typeof fetchDataForUser === 'function') {
    ...
    if (loading && isInitialMount.current) return <LoadingSpinner fullScreen message="Cargando infraestructura..." />;
    
    return (
        <div className="container mx-auto p-4 md:p-6 lg:p-8">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Gestión de Tanques y Sensores</h1>
                <button onClick={() => handleOpenModal('create-tank', { userId: selectedUserId })} className="flex items-center px-4 py-2 bg-[#39A900] text-white rounded-lg hover:bg-[#2F8B00] transition-colors shadow-md">
                    <MapPin className="w-5 h-5 mr-2" /><span>Nuevo Tanque</span>
                </button>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex items-center"><LayoutGrid className="h-8 w-8 text-blue-500" /><div className="ml-4"><p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Tanques</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalTanks}</p></div></div>
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex items-center"><Cpu className="h-8 w-8 text-indigo-500" /><div className="ml-4"><p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Sensores</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalSensors}</p></div></div>
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex items-center"><CheckCircle2 className="h-8 w-8 text-green-500" /><div className="ml-4"><p className="text-sm font-medium text-gray-500 dark:text-gray-400">Tanques Activos</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.activeTanks}</p></div></div>
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 flex items-center"><WarningIcon className="h-8 w-8 text-yellow-500" /><div className="ml-4"><p className="text-sm font-medium text-gray-500 dark:text-gray-400">En Mantenimiento</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.maintenanceTanks}</p></div></div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {isAdmin && users.length > 1 && (
                        <div className="relative">
                            ...
                            <select value={selectedUserId || ''} onChange={(e) => setSelectedUserId(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-[#39A900] focus:border-[#39A900]">
                                {users.map(user => (<option key={user.id} value={user.id}>{user.name}</option>))}
                            </select>
                        </div>
                    )}
                    <div className={`relative ${isAdmin && users.length > 1 ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                        <input type="text" placeholder="Buscar por nombre o ubicación..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-[#39A900] focus:border-[#39A900]"/>
                    </div>
                    <div className="relative">
                        <SlidersHorizontal className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="w-full pl-10 pr-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-[#39A900] focus:border-[#39A900]">
            ...
            {error && <div className="text-center text-red-500 p-4 bg-red-50 dark:bg-red-900/20 rounded-md flex items-center justify-center"><AlertCircle className="w-6 h-6 mr-2"/> {error}</div>}
            
            <div className="space-y-4">
                {!loading && filteredTanks.length === 0 ? (
                    <div className="text-center py-16 text-gray-500 border-2 border-dashed rounded-lg bg-white dark:bg-gray-800"><MapPin className="w-12 h-12 mx-auto mb-4" /><h3 className="text-lg font-semibold">No se encontraron tanques</h3><p className="mt-1 text-sm">Prueba a cambiar los filtros o crea un nuevo tanque.</p></div>
                    ...
                    return (
                        <div key={tank.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
                            <header className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50" onClick={() => setExpandedTanks(prev => { const next = new Set(prev); next.has(tank.id) ? next.delete(tank.id) : next.add(tank.id); return next; })}>
                                <div className="flex items-center space-x-4"><MapPin className="w-8 h-8 text-[#39A900]" /><div><h3 className="font-semibold text-lg text-gray-900 dark:text-white">{tank.name}</h3><p className="text-sm text-gray-500 dark:text-gray-400">{tank.location}</p></div>{getStatusChip(tank.status)}</div>
    ...
    return (
        <div className="flex flex-col justify-between p-4 bg-white dark:bg-gray-900 rounded-lg border dark:border-gray-700 shadow h-full">
            ...
            <div className="flex justify-center space-x-2 mt-4 pt-3 border-t dark:border-gray-700">
                <button onClick={() => onOpenModal('view-sensor', sensor)} className="p-2 text-gray-500 hover:text-blue-500 rounded-full" title="Ver Detalles"><Eye className="w-4 h-4"/></button>
                <button onClick={() => onOpenModal('edit-sensor', { sensor })} className="p-2 text-gray-500 hover:text-green-500 rounded-full" title="Editar Sensor"><Edit className="w-4 h-4"/></button>
                <button onClick={() => onDelete('sensor', sensor)} className="p-2 text-gray-500 hover:text-red-500 rounded-full" title="Eliminar Sensor"><Trash2 className="w-4 h-4"/></button>
            </div>
        </div>
    );
};