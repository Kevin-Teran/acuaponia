 ...
 * @description Interfaz de chat definitiva con layout de altura completa, scroll interno,
 * pre-filtro de mensajes y estilos pulidos.
 * @author Tu Nombre
 * @version 10.0.0 (Definitive & Final)
 * @date 2025-08-20
 */
 'use client';
 ...
 const AsistenteIAPage: React.FC = () => {
   ...
   const sendMessage = useCallback(async (pregunta: string) => {
     ...
     // MEJORADO: Prefiltro de mensajes más robusto
     ...
     try {
       ...
       const aiResponseText = response.ok ? data.respuesta : `Error: ${data.error || 'No se pudo obtener una respuesta.'}`;
       ...
       setMessages(prev => [...prev, { id: Date.now() + 1, text: errorMsg, sender: 'ai' }]);
     } finally {
       setCargando(false);
       inputRef.current?.focus();
     }
   ...
   return (
     // SOLUCIÓN DE LAYOUT: Este contenedor "cancela" el padding del layout (-m-6) y toma control total de la altura (h-full).
     <div className="flex flex-col h-full -m-6 animate-in fade-in duration-500">
       ...
       {/* SOLUCIÓN DE LAYOUT: Este div envuelve la tarjeta y le aplica el padding que antes estaba en el layout. */}
       <div className="flex flex-col flex-grow p-6 pt-2 overflow-hidden">
         <Card className="flex flex-col flex-grow p-0 overflow-hidden w-full">
           {/* SOLUCIÓN DE LAYOUT: Esta área tiene su propio scroll y no depende del scroll principal. */}
           ...
           {/* SOLUCIÓN DE LAYOUT: Esta área se mantiene anclada abajo. */}
