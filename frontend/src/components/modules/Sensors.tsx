...
export const Sensors: React.FC = () => {
  ...
  const [tankFormData, setTankFormData] = useState({ name: '', location: '', capacity: 1000, currentLevel: 500, status: 'ACTIVE' as Tank['status'] });
  ...
  const handleOpenTankModal = (tank?: Tank) => {
    if (tank) {
      setEditingTank(tank);
      setTankFormData({ name: tank.name, location: tank.location, capacity: tank.capacity, currentLevel: tank.currentLevel, status: tank.status });
    } else {
      setEditingTank(null);
      setTankFormData({ name: '', location: '', capacity: 1000, currentLevel: 500, status: 'ACTIVE' });
    }
  ...
  const handleOpenSensorModal = (sensor?: Sensor, tankId?: string) => {
    if (sensor) {
        ...
        setSensorFormData({ name: '', type: 'TEMPERATURE', tankId: tankId || (tanks[0]?.id ?? ''), calibrationDate: new Date().toISOString().split('T')[0] });
    }
  ...
  const handleTankSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (editingTank) {
            await tankService.updateTank(editingTank.id, tankFormData);
        } else {
            await tankService.createTank(tankFormData as any);
        }
  ...
  const handleDeleteTank = (tank: Tank) => {
    Swal.fire({
      ...
      text: "Esta acción también eliminará todos los sensores asociados. No se puede deshacer.",
      icon: 'warning',
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        <div className="flex items-center space-x-3">
            ...
            <button onClick={() => handleOpenSensorModal()} disabled={tanks.length === 0} title={tanks.length === 0 ? "Debe crear un tanque primero" : "Añadir nuevo sensor"} className="bg-sena-green hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors shadow-md disabled:bg-gray-400 disabled:cursor-not-allowed"><Cpu className="w-5 h-5" /><span>Nuevo Sensor</span></button>
        </div>
      </div>
            ...
            return (
                <Card key={tank.id} className="p-0 overflow-hidden">
                    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 cursor-pointer" onClick={() => toggleTankExpansion(tank.id)}>
                        ...
                        <div className="flex items-center space-x-4">
                            <span className="text-sm text-gray-600 dark:text-gray-300">{tankSensors.length} Sensores</span>
                                        ...
                                        <div key={sensor.id} className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col justify-between">
                                            <div>
                                                ...
                                                <div className="space-y-2 text-sm">
                                                    ...
                                                    <div className="flex justify-between"><span className="text-gray-500">Batería:</span><span className="font-medium text-gray-900 dark:text-white">{sensor.batteryLevel}%</span></div>
                                                </div>
                                            </div>
      ...
      <Modal title={editingTank ? "Editar Tanque" : "Nuevo Tanque"} isOpen={showTankModal} onClose={handleCloseModals}>
        <form onSubmit={handleTankSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Capacidad (Litros)</label><input type="number" min="0" value={tankFormData.capacity} onChange={(e) => setTankFormData({ ...tankFormData, capacity: Number(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sena-green bg-white dark:bg-gray-700" required /></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nivel Actual (Litros)</label><input type="number" min="0" value={tankFormData.currentLevel} onChange={(e) => setTankFormData({ ...tankFormData, currentLevel: Number(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sena-green bg-white dark:bg-gray-700" required /></div>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-4"><button type="button" onClick={handleCloseModals} className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">Cancelar</button><button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-sena-green hover:bg-green-700 text-white rounded-lg flex items-center space-x-2 transition-colors disabled:bg-green-300">{isSubmitting ? <Loader className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}<span>{editingTank ? 'Guardar Cambios' : 'Crear Tanque'}</span></button></div>
        </form>
      </Modal>

      <Modal title={editingSensor ? "Editar Sensor" : "Nuevo Sensor"} isOpen={showSensorModal} onClose={handleCloseModals}>
        <form onSubmit={handleSensorSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo de Sensor</label><select value={sensorFormData.type} onChange={(e) => setSensorFormData({ ...sensorFormData, type: e.target.value as any })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sena-green bg-white dark:bg-gray-700"><option value="TEMPERATURE">Temperatura</option><option value="PH">pH</option><option value="OXYGEN">Oxígeno</option></select></div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Asignar a Tanque</label><select value={sensorFormData.tankId} onChange={(e) => setSensorFormData({ ...sensorFormData, tankId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-sena-green bg-white dark:bg-gray-700" required>{tanks.map(tank => <option key={tank.id} value={tank.id}>{tank.name}</option>)}</select></div>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-4"><button type="button" onClick={handleCloseModals} className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">Cancelar</button><button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-sena-green hover:bg-green-700 text-white rounded-lg flex items-center space-x-2 transition-colors disabled:bg-green-300">{isSubmitting ? <Loader className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}<span>{editingSensor ? 'Guardar Cambios' : 'Crear Sensor'}</span></button></div>
        </form>
      </Modal>
    </div>
  );
