 ...
 * @description Componente que renderiza una serie de tarjetas de estado para cada sensor activo.
 * Cada tarjeta muestra información vital como la última lectura, tendencia y umbrales.
 */
 import React from 'react';
 import { Thermometer, Droplets, Wind, Zap, BarChartHorizontal, Clock, ServerOff, Activity, TrendingUp, TrendingDown, Minus, ArrowDownCircle, ArrowUpCircle } from 'lucide-react';
         ...
         <Card className="flex flex-col h-full p-4">
             <div className="flex-grow flex flex-col">
                 <div className="flex items-center justify-between mb-4">
                     <div className="flex items-center space-x-3">
                         <div className={clsx('p-2 rounded-full', bg)}>
                             <Icon className={clsx('w-5 h-5', color)} />
                         </div>
                         <h3 className="font-semibold text-lg text-gray-900 dark:text-white">{name}</h3>
                     </div>
                     <span className={clsx('px-2.5 py-1 rounded-full text-xs font-medium', statusInfo.bg, statusInfo.text)}>{sensor.readingStatus}</span>
                 </div>
             ...
             <div className="space-y-3 mt-auto">
                 ...
                 <div className="flex flex-col items-center justify-center text-xs pt-2 space-y-1">
                     {hasReading && (
                         <div className={clsx('flex items-center font-medium', trendInfo.color)}>
