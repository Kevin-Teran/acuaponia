 ...
 */
 ...
 import { useAuth } from '@/context/AuthContext'; // ✅ CORRECCIÓN: Importar desde el contexto
 ...
 export const LoginForm: React.FC = () => {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
     ...
     if (password.length < 6) {
       Swal.fire({ icon: 'info', title: 'Contraseña Corta', text: 'La contraseña debe tener al menos 6 caracteres.', confirmButtonColor: senaGreen });
       return;
     }
     ...
     try {
       // ✅ Pasamos el objeto completo que espera la función 'login'
       await login({ identifier, password, rememberMe });
     } catch (error: any) {
       Swal.fire({ icon: 'error', title: 'Acceso Denegado', text: error.message, confirmButtonColor: senaGreen });
     } finally {
       setIsLoading(false);
     }
   };
   ...
   // ✅ CORRECCIÓN: Usa la contraseña real de la base de datos
   const demoAccounts = [
     { email: 'admin@sena.edu.co', password: 'password123', role: 'Administrador' },
     { email: 'usuario@sena.edu.co', password: 'password123', role: 'Usuario' }
   ];
     ...
     <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-800 dark:to-black flex items-center justify-center p-4 transition-colors duration-300">
       <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-5 duration-500">
         <div className="text-center mb-8">
           <Image src="/logo-sena.png" alt="Logo SENA" width={80} height={80} className="mx-auto mb-4" />
           ...
           <form onSubmit={handleSubmit} className="space-y-6">
             <div>
               ...
               <div className="relative">
                 ...
                 <input id="email" type="email" value={identifier} onChange={(e) => setIdentifier(e.target.value)} autoComplete="email" className="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 bg-white/50 dark:bg-gray-700/50 text-gray-900 dark:text-white transition" placeholder="usuario@sena.edu.co" required />
               </div>
             </div>
             <div>
               ...
               <div className="relative">
                 ...
                 <input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" className="w-full pl-10 pr-12 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 bg-white/50 dark:bg-gray-700/50 text-gray-900 dark:text-white transition" placeholder="Mínimo 6 caracteres" required />
                 <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"}>
             ...
             <div className="flex items-center">
               ...
               <label htmlFor="remember-me" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">Recordar mi sesión</label>
             </div>
         ...
         <p className="text-center text-xs text-gray-500 dark:text-gray-400 mt-8">© {new Date().getFullYear()} SENA - Todos los derechos reservados</p>
       </div>
     </div>
   );
 };
